...
export default function TugasAnswerShow({ auth }) {
    const { tugases, kelompoks: kelompok, users } = usePage().props;
    ...
    return (
        <AuthenticatedLayout back userLogin={auth.user}>
            <div className="bg-gray-50 p-6 rounded-xl space-y-4">
                <IconTitle title={`Hasil Tugas Kelompok ${kelompok.name}`}>
                    <svg
                        ...
                        {tugases.map((tugas, index) => {
                            return (
                                <TableRow key={index}>
                                    ...
                                    />
                                    <TableData
                                        children={`${tugas.answers.length}/${tugas.kelompoks.capacity}`}
                                    />
                                    <TableData
                                        children={
                                            <Link
                                                href={route(
                                                    "tugas-answer-guru.jawabanSiswa",
                                                    tugas.id
                                                )}
                                            >
                                                <PrimaryButton
