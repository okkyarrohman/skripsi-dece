...
export default function AbsenEdit({ auth }) {
    const { absens } = usePage().props;

    const [openStatus, setOpenStatus] = useState(false);
    const [selectedOptionStatus, setSelectedOptionStatus] = useState(
        absens.is_active === "Y" ? "Aktif" : "Nonaktif"
    );

    const { data, setData, post, errors } = useForm({
        _method: "patch",
        name: absens.name,
        meet_date: absens.meet_date,
        is_active: absens.is_active,
    });

    const handleOnSubmit = (e) => {
        e.preventDefault();
        post(route("absen-guru.update", absens.id));
    };
