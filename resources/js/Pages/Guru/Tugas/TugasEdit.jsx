...
export default function TugasEdit({ auth }) {
    const { tugases, kelompoks } = usePage().props;

    const filteredKelompoks = kelompoks.filter(
        (kelompok) => kelompok.is_active == "Y"
    );

    const filteredByIdKelompoks = kelompoks.find(
        (kelompok) => kelompok.id == tugases.kelompok_id
    );

    const [openKelompok, setOpenKelompok] = useState(false);
    const [selectedOptionKelompok, setSelectedOptionKelompok] = useState(
        filteredByIdKelompoks ? filteredByIdKelompoks.name : ""
    );
    const [openStatus, setOpenStatus] = useState(false);
    const [selectedOptionStatus, setSelectedOptionStatus] = useState(
        tugases.is_active === "Y" ? "Aktif" : "Nonaktif"
    );

    const { data, setData, post, errors } = useForm({
        _method: "patch",
        kelompok_id: tugases.kelompok_id,
        is_active: tugases.is_active,
        name: tugases.name,
        description: tugases.description,
        deadline_date: tugases.deadline_date,
        deadline_time: tugases.deadline_time,
        question_1: tugases.question_1,
        question_2: tugases.question_2,
        question_3: tugases.question_3,
    });
    ...
    return (
        <AuthenticatedLayout back userLogin={auth.user}>
            <div className="p-6 rounded-xl bg-gray-50 space-y-6">
                <IconTitle title="Edit Tugas Kelompok">
                    <svg
